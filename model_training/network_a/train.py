...
from model_training.network_a.sync_topology import sync_topology_if_needed
...
def _accuracy(model: NetworkA, loader, device: torch.device, mode: str) -> float:
    model.eval()
    correct = 0
    total = 0
    with torch.no_grad():
...
def _calibrate_plan(model: NetworkA, loader, device: torch.device, n: int = 200):
    model.eval()
    max_pool = 0.0
    max_fc1 = 0.0
    max_fc2 = 0.0
