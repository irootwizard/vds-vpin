...
def setup():
    ...
    # 确保 A0 与 n 互质（属于 Z_n*）
    # 这是必需的，因为在某些操作中（如 prove_non_membership）可能需要计算 A0 的模逆
    # 注意：A0 与 n 不互质的概率约为 2/2^1536，几乎不可能发生，但添加最大尝试次数以防万一
    import math
    max_attempts = 1000  # 最大尝试次数（理论上几乎总是第一次就成功）
    for attempt in range(max_attempts):
        A0 = secrets.randbelow(n)
        if math.gcd(A0, n) == 1:
            break
    else:
        # 如果 1000 次都失败（概率极低，几乎不可能），抛出异常
        raise RuntimeError(f"无法找到与 n 互质的 A0，已尝试 {max_attempts} 次。这通常不应该发生。")
    return n, A0, dict()
