import sys
sys.set_int_max_str_digits(20000)  # Increase limit for large integer string conversion
import time
# Note: All timing measurements use time.time() which returns seconds (float).
# All time differences are in seconds.
...
def test_mining(total_utxo_set_size_for_merkle_tree, total_utxo_set_size_for_accumulator, num_of_inputs_in_tx, num_of_outputs_in_tx, num_of_txs_in_block):
    ...
    tik = time.time()  # time.time() returns seconds since epoch (float)
    for i in range(len(elements_for_merkle_tree)):
        merkle_tree.add_leaf(str(i), True)
    merkle_tree.make_tree()
    tok = time.time()
    merkle_add_timing.append(tok - tik)  # Time difference in seconds
    print("<--   Done.", merkle_add_timing[-1], "seconds")
    ...
    tok = time.time()
    acc_batch_add_genesis_timing.append(tok - tik)  # Time difference in seconds
    print("<--   Done.", acc_batch_add_genesis_timing[-1], "seconds")
    ...
    sum_times = sum(times)
    merkle_proof_timing.append(sum_times / num_of_inputs_in_tx)  # average ; per tx (in seconds)
    print("<--   Done. total:", sum_times, "seconds; per tx:", merkle_proof_timing[-1], "seconds")
    ...
    for i in range(num_of_txs_in_block):
        ...
        times.append(tok - tik)
    sum_times = sum(times)
    acc_batch_prove_mem_timing.append(sum_times / len(times))  # average (in seconds)
    print("<--   Done. total:", sum_times, "seconds; per tx:", acc_batch_prove_mem_timing[-1], "seconds")
    ...
    for i in range(num_of_txs_in_block):
        ...
        times.append(tok - tik)
    sum_times = sum(times)
    acc_batch_prove_mem_with_NIPoE_timing.append(sum_times / len(times))  # average (in seconds)
    print("<--   Done. total:", sum_times, "seconds; per tx:", acc_batch_prove_mem_with_NIPoE_timing[-1], "seconds")
    ...
    tok = time.time()
    merkle_verify_mem_per_block_timing.append(tok - tik)  # Time in seconds
    merkle_verify_mem_per_tx_timing.append((tok - tik) / num_of_txs_in_block)  # average (in seconds)
    print("<--   Done. total (per block):", merkle_verify_mem_per_block_timing[-1], "seconds; per tx:", merkle_verify_mem_per_tx_timing[-1], "seconds")
    ...
    tok = time.time()
    acc_batch_verify_mem_per_block_timing.append(tok - tik)  # Time in seconds
    acc_batch_verify_mem_per_tx_timing.append((tok - tik) / num_of_txs_in_block)  # average (in seconds)
    print("<--   Done. total (per block):", acc_batch_verify_mem_per_block_timing[-1], "seconds; per tx:", acc_batch_verify_mem_per_tx_timing[-1], "seconds")
    ...
    for i in range(num_of_txs_in_block):
        ...
        assert batch_verify_membership_with_NIPoE(
            ...
            n)
    tok = time.time()
    acc_batch_verify_mem_with_NIPoE_per_block_timing.append(tok - tik)  # Time in seconds
    acc_batch_verify_mem_with_NIPoE_per_tx_timing.append((tok - tik) / num_of_txs_in_block)  # average (in seconds)
    print("<--   Done. total (per block):", acc_batch_verify_mem_with_NIPoE_per_block_timing[-1], "seconds; per tx:", acc_batch_verify_mem_with_NIPoE_per_tx_timing[-1], "seconds")
    ...
    tok = time.time()
    acc_batch_delete_timing.append(tok - tik)  # Time in seconds
    print("<--   Done.", acc_batch_delete_timing[-1], "seconds")
    ...
    tok = time.time()
    acc_batch_add_per_block_timing.append(tok - tik)  # Time in seconds
    print("<--   Done.", acc_batch_add_per_block_timing[-1], "seconds")
    ...
    tok = time.time()
    acc_batch_verify_two_NIPoE_post_mining.append(tok - tik)  # Time in seconds
    print("<--   Done.", acc_batch_verify_two_NIPoE_post_mining[-1], "seconds")
...
with open(GENERATED_CSV_DIRECTORY + '/proofs-per-tx.csv', mode='w') as csv_file:
    ...
    csv_file.writerow(['# Time unit: seconds'] + [''] * (len(num_of_txs_in_block) - 1))
