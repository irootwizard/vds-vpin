...
if (Test-Path $bin) {
    ...
    Fail "Missing table.bin - set VPIN_ARTIFACTS_BASE_URL and run scripts\pull-runtime-artifacts.ps1"
} else {
    Warn "Missing table.bin - run scripts\pull-runtime-artifacts.ps1 or set VPIN_ARTIFACTS_BASE_URL"
}

$hasW = Test-VpinModelWeights -RepoRoot $ROOT
