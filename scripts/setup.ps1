...
if (-not $pyCmd) {
    Write-Host "[ERROR] 未找到 python，请安装 Python 3.11.x 并加入 PATH" -ForegroundColor Red
    Write-Host "       版本基线见 docs/开发环境完整配置指南.md" -ForegroundColor Yellow
    exit 1
}
$ver = & python -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')"
if ([double]$ver -lt 3.11 -or [double]$ver -ge 3.12) {
    Write-Host "[ERROR] 全栈基线需要 Python 3.11.x，当前 $ver" -ForegroundColor Red
    Write-Host "       请安装 3.11（推荐 3.11.7），删除 .venv 后重跑本脚本" -ForegroundColor Yellow
    Write-Host "       详见 docs/开发环境完整配置指南.md 「版本基线」" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "  .\scripts\pull-runtime-artifacts.ps1  # 远程拉取 table.bin（需 VPIN_ARTIFACTS_BASE_URL）" -ForegroundColor White
Write-Host "  .\scripts\build-release.ps1        # 打包发布（含权重，不含 BSGS 表）" -ForegroundColor White
Write-Host "  .\start-ahe.ps1                    # Python + Tauri（前台）" -ForegroundColor White
Write-Host "  .\scripts\start-ahe-full.ps1       # 三引擎 + Tauri（独立窗口）" -ForegroundColor White
