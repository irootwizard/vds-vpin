...
Verification path (matches paper §3 transcript order
``cm_W → cm_x → γ → γ_add → γ_mult → sub_circuit``):

1. Bundle schema sanity (kind=``spartan_pc``, opening length matches the
   advertised ``num_scalars``).
2. cm_W catalog binding — the bundle's ``cm_w.cm_hex`` must equal the
   value persisted in the local model catalog (a hex string the client
   stored at registration time, e.g. ``TOY_W_STAR_CM_HEX``).
3. Pedersen digest cross-check — the bundle's legacy
   ``model_commitment.cm_weights.digest_hex`` must be derivable from the
   W* opening (re-uses :func:`vpin_client.commitment.pedersen.scalars_digest`).
4. L1 binding — toy weight layout slots match trace filter / weights /
   bias values.
5. M1 scalar checks — run the existing :func:`verify_all_client` stack on
   the supplied traces with γ / γ_add / γ_mult.
6. γ-replay rejection — a :class:`GammaReplayGuard` records every
   (session_id, γ) pair the client has ever accepted and refuses a
   repeat.
...
from vpin_client.commitment.pedersen import scalars_digest
...
class GammaReplayGuard:
    """Reject (session_id, γ) pairs the client has already accepted.

    Keeps a small in-memory set; production deployments persist to disk /
    catalog. The CSPRNG sampled γ MUST appear at most once per session id.
    """
    ...
    def admit(self, session_id: str, gamma_hex: str) -> None:
        ...
        if key in self._seen:
            raise CpsVerifyError(
                ...
                f"γ {gamma_hex[:8]}... already used in session {session_id}",
            )
        self._seen.add(key)
...
def stack_from_toy_traces(
    ...
    # Toy FC is a 1→2 linear layer with bias; spec expects weights_in_out
...
def verify_toy_cps_bundle(
    ...
    *,
    ...
    """P0–P6 client-local verification of a toy CPS bundle.
    ...
    try:
        ...
        scalar_ok = True
    except Exception as exc:  # noqa: BLE001 — bubble as CpsVerifyError
