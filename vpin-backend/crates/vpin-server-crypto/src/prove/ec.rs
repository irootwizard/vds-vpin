...
use crate::circuit_prove::{prove_point_add, prove_point_mult, verify_point_add, verify_point_mult};
...
pub fn verify_ec_bundle(
    ...
    let add_ok = match &bundle.point_add {
        Some(p) => verify_point_add(p, network, model, input, challenge).is_ok(),
        None => true,
    };
    let mult_ok = match &bundle.point_mult {
        Some(p) => verify_point_mult(p, network, model, input, challenge).is_ok(),
        None => true,
    };
    add_ok && mult_ok
}
