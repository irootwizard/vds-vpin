//! Z.2 acceptance tests for the toy sum pool R1CS (Eq. 7).
...
use vpin_server_crypto::circuit::layer::pool_sum::{prove_pool_toy, verify_pool_toy, PoolToyTrace};
...
#[test]
fn pool_toy_prove_verify_roundtrip_honest() {
    let trace = toy_trace();
    let (model, input) = toy_commitments();
    let challenge = fixed_challenge();
    let (proof, prove_ms) = prove_pool_toy(&trace, &model, &input, &challenge).expect("prove");
    let t0 = Instant::now();
    verify_pool_toy(&proof, &model, &input, &challenge).expect("verify");
    write_perf("Z-2", prove_ms, t0.elapsed().as_millis(), proof.proof_bytes.len());
}

#[test]
fn pool_toy_rejects_wrong_challenge() {
    let trace = toy_trace();
    let (model, input) = toy_commitments();
    let challenge = fixed_challenge();
    let (proof, _) = prove_pool_toy(&trace, &model, &input, &challenge).expect("prove");
    let mut bad = challenge.clone();
    bad.gamma = "ff".repeat(32);
    assert!(verify_pool_toy(&proof, &model, &input, &bad).is_err());
}

#[test]
fn pool_toy_rejects_tampered_proof_bytes() {
    let trace = toy_trace();
    let (model, input) = toy_commitments();
    let challenge = fixed_challenge();
    let (mut proof, _) = prove_pool_toy(&trace, &model, &input, &challenge).expect("prove");
    if let Some(last) = proof.proof_bytes.last_mut() {
        *last ^= 0xff;
    }
    assert!(verify_pool_toy(&proof, &model, &input, &challenge).is_err());
}

#[test]
fn pool_toy_rejects_wrong_model_commitment() {
    let trace = toy_trace();
    let (model, input) = toy_commitments();
    let challenge = fixed_challenge();
    let (proof, _) = prove_pool_toy(&trace, &model, &input, &challenge).expect("prove");
    ...
    assert!(verify_pool_toy(&proof, &bad_model, &input, &challenge).is_err());
}

#[test]
fn pool_toy_prove_rejects_wrong_input_witness() {
    let mut bad = toy_trace();
    bad.windows[0][0] = 99;
    let (model, input) = toy_commitments();
    let challenge = fixed_challenge();
    let res = prove_pool_toy(&bad, &model, &input, &challenge);
    assert!(matches!(res, Err(ref e) if e.contains("unsatisfied")));
}

#[test]
fn pool_toy_protocol_rejects_circuit_name_mismatch() {
    let trace = toy_trace();
    let (model, input) = toy_commitments();
    let challenge = fixed_challenge();
    let (mut proof, _) = prove_pool_toy(&trace, &model, &input, &challenge).expect("prove");
    proof.circuit_name = "spoof_pool".to_string();
    assert!(verify_pool_toy(&proof, &model, &input, &challenge).is_err());
}
