"""
Encrypt / decrypt / fixed-point — semantic port of src/cnn_networks/Client.py.
"""
...
from vpin_backend.crypto.ahe.curve import KeyMaterial
...
def encrypt_tensor(
    ...
    """layout: '4d' (NCHW batch=1) or '2d' (matrix)."""
    if layout == "4d":
        shape = tensor.shape
...
def decrypt_tensor(
    ...
    if layout == "4d":
        ...
        for i in range(enc_c1.shape[0]):
            for j in range(enc_c1.shape[1]):
                for k in range(enc_c1.shape[2]):
                    for l in range(enc_c1.shape[3]):
                        out[i, j, k, l] = decrypt_ciphertext_pair(
                            private_scalar,
                            enc_c1[i, j, k, l],
                            enc_c2[i, j, k, l],
                            generator,
    ...
    for i in range(rows):
        for j in range(cols):
            out[i, j] = decrypt_ciphertext_pair(
                private_scalar,
                enc_c1[i, j],
                enc_c2[i, j],
                generator,
