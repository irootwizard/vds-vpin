...
def embed_u128_to_scalar(value: int) -> Scalar:
    """Canonical u128 鈫?scalar (16-byte LE in low half of 64-byte wide buffer, mod q_1)."""
...
def fold_rlc(values: list[int], gamma: Scalar) -> Scalar:
    """危_i 纬^i 路 embed(v_i) 鈥?paper RLC fold."""
    acc = 0
    pow_val = 1
    for v in values:
...
def mac_filter_window(filter_flat: list[int], window: list[int]) -> Scalar:
    """Dot product 危_k filter[k] 路 window[k] (paper Eq. 5 / 6 per window)."""
...
def conv_rlc_right(
    ...
    """Paper Eq. (9) RHS: 危_i 纬^i 路 MAC(f, window_i)."""
    acc = 0
    pow_val = 1
...
def fc_rlc_right(
    ...
    """FC Eq. (10) RHS: 危_k d[k]路(危_i 纬鈥瞊i路W[k,i]) + 危_j 纬鈥瞊j路b[j]."""
    acc = 0
